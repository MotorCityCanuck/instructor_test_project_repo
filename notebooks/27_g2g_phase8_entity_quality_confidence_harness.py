"""Databricks harness for Silver-to-Gold Phase 8 quality-confidence testing."""
...
PHASE8_TARGET_CONFIG = {
    "entity_data_quality_confidence": {
        ...
        + int(spark.table(f"{environment.catalog}.{environment.silver_schema}.teams").count()),
        "reconciliation_name": "entity_data_quality_confidence_row_balance",
    },
}
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Validate Silver-to-Gold Phase 8 quality confidence in Databricks."
    )
...
def main() -> None:
    ...
    try:
        ...
        validated_columns = validate_phase8_source_contract(spark, environment)
        ...
        for table_name in PHASE8_TARGET_TABLES:
            started_ts = utc_now()
            ...
            append_records(
                ...
                [
                    build_table_run_start_record(
                        ...
                        build_stage="quality",
                        build_order=PHASE8_TARGET_CONFIG[table_name]["build_order"],
                        started_ts=started_ts,
                    )
                ],
            )

        publication_summary = publish_phase8_quality_table(
            spark,
            environment,
            analysis_as_of_date=pipeline_context.analysis_as_of_date,
            quality_rules_config=config.data["quality_rules"],
        )

        source_counts = {
            table_name: PHASE8_TARGET_CONFIG[table_name]["source_count_resolver"](
                spark,
                environment,
            )
            for table_name in PHASE8_TARGET_TABLES
        ...
        output_counts = {
            "entity_data_quality_confidence": publication_summary.entity_data_quality_confidence.output_row_count,
        }
        excluded_counts = {
        ...
        append_records(
            ...
            [
                build_reconciliation_record(
                    pipeline_context,
                    reconciliation_name=PHASE8_TARGET_CONFIG[table_name]["reconciliation_name"],
        ...
        print("Validated Phase 8 source columns:")
        for logical_name, columns in validated_columns.items():
            layer, table_name, _required_columns = PHASE8_REQUIRED_SOURCE_COLUMNS[logical_name]
            ...
            for column_name in columns:
                print(f"      * {column_name}")
        print("Planned Phase 8 target tables:")
        for table_name in PHASE8_TARGET_TABLES:
            print(f"  - {environment.catalog}.{environment.gold_schema}.{table_name}")
        print("Published Phase 8 target tables:")
        print(
            f"  - entity_data_quality_confidence: "
            f"{publication_summary.entity_data_quality_confidence.target_table_fqn} "
            f"(input={source_counts['entity_data_quality_confidence']}, "
            f"output={output_counts['entity_data_quality_confidence']})"
        )
        ...
        set_task_value(
            dbutils,
            "phase8_entity_data_quality_confidence_row_count",
            output_counts["entity_data_quality_confidence"],
        )
