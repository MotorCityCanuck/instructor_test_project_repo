"""Preview the Bronze-to-Silver competition-stage build configuration."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Preview the Bronze-to-Silver competition-stage build configuration."
    )
...
def main() -> None:
    ...
    table_names = get_stage_table_names(config, "competition")

    print(f"Script version: {SCRIPT_VERSION}")
    print(f"Release name: {args.release_name}")
    print(f"Run ID: {args.run_id}")
    print("Competition tables:")
    ...
    set_task_value(dbutils, "competition_table_count", len(table_names))
