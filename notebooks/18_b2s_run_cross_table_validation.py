"""Run Bronze-to-Silver cross-table validation in wiring-preview mode."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run Bronze-to-Silver cross-table validation in wiring-preview mode."
    )
...
def main() -> None:
    ...
    print(f"Configured convenience views enabled: {config.data['execution']['create_convenience_views']}")
    print("Cross-table validation module and convenience-view names are available:")
    for view_name in CONVENIENCE_VIEW_NAMES:
        print(f"- {view_name}")
    print("This task currently validates workflow wiring and module availability only.")

    set_task_value(dbutils, "run_id", args.run_id)
    set_task_value(dbutils, "cross_table_validation_ready", True)
