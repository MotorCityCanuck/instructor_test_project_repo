"""Databricks harness for Silver-to-Gold Phase 6 player feature testing."""
...
from napa_pipeline.silver_to_gold.features import get_player_feature_registry
...
from napa_pipeline.silver_to_gold.io import get_gold_target_table_fqn, get_operations_table_fqn
...
PHASE6_TARGET_CONFIG = {
    "player_performance_features": {
        "build_order": 70,
        "source_count_resolver": lambda spark, environment, evidence_windows_count: int(
            spark.table(get_gold_target_table_fqn(environment, "player_current_ratings")).count()
        )
        * evidence_windows_count,
        "reconciliation_name": "player_performance_features_row_balance",
    },
    "player_development_features": {
        "build_order": 80,
        "source_count_resolver": lambda spark, environment, evidence_windows_count: int(
            spark.table(f"{environment.catalog}.{environment.silver_schema}.players").count()
        ),
        "reconciliation_name": "player_development_features_row_balance",
    },
}
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Validate Silver-to-Gold Phase 6 player features in Databricks."
    )
...
def main() -> None:
    ...
    try:
            ...
            append_records(
                ...
                [
                    build_table_run_start_record(
                        ...
                        build_stage="player_features",
                        build_order=PHASE6_TARGET_CONFIG[table_name]["build_order"],
                        started_ts=started_ts,
                    )
                ],
            )

        publication_summary = publish_phase6_feature_tables(
            spark,
            environment,
        ...
        source_counts = {
            table_name: PHASE6_TARGET_CONFIG[table_name]["source_count_resolver"](
                spark,
                environment,
                evidence_windows_count,
            )
        ...
        output_counts = {
            "player_performance_features": publication_summary.player_performance_features.output_row_count,
            "player_development_features": publication_summary.player_development_features.output_row_count,
        }
        excluded_counts = {
            table_name: source_counts[table_name] - output_counts[table_name]
            for table_name in PHASE6_TARGET_TABLES
        }
        if excluded_counts["player_performance_features"] != 0:
            raise ValueError(
                "player_performance_features row count did not reconcile with "
                "player_current_ratings multiplied by the configured windows: "
                f"source_rows={source_counts['player_performance_features']}, "
                f"output_rows={output_counts['player_performance_features']}."
            )
        if excluded_counts["player_development_features"] != 0:
            raise ValueError(
                "player_development_features row count did not reconcile with players: "
                f"source_rows={source_counts['player_development_features']}, "
                f"output_rows={output_counts['player_development_features']}."
            )
        ...
        append_records(
            ...
            [
                build_reconciliation_record(
                    pipeline_context,
                    reconciliation_name=PHASE6_TARGET_CONFIG[table_name]["reconciliation_name"],
        ...
        print("Validated Phase 6 source columns:")
        for logical_name, columns in validated_columns.items():
            layer, table_name, _required_columns = PHASE6_REQUIRED_SOURCE_COLUMNS[logical_name]
        ...
        print("Planned Phase 6 target tables:")
        for table_name in PHASE6_TARGET_TABLES:
            print(f"  - {environment.catalog}.{environment.gold_schema}.{table_name}")
        print("Published Phase 6 target tables:")
        print(
            f"  - player_performance_features: "
            f"{publication_summary.player_performance_features.target_table_fqn} "
            f"(input={source_counts['player_performance_features']}, "
            f"output={output_counts['player_performance_features']})"
        )
        print(
            f"  - player_development_features: "
            f"{publication_summary.player_development_features.target_table_fqn} "
            f"(input={source_counts['player_development_features']}, "
            f"output={output_counts['player_development_features']})"
        )
        ...
        set_task_value(dbutils, "phase6_feature_registry_count", len(feature_registry))
        set_task_value(
            dbutils,
            "phase6_player_performance_features_row_count",
            output_counts["player_performance_features"],
        )
        set_task_value(
            dbutils,
            "phase6_player_development_features_row_count",
            output_counts["player_development_features"],
        )
