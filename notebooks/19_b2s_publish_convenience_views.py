"""Publish Bronze-to-Silver convenience views."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Publish Bronze-to-Silver convenience views."
    )
...
def main() -> None:
    ...
    print("Published convenience views:")
    for view_name, row_count in published_counts.items():
        print(f"- {view_name} rows={row_count}")

    set_task_value(dbutils, "run_id", args.run_id)
    set_task_value(dbutils, "convenience_view_count", len(published_counts))
