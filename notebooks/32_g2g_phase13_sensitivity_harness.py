"""Databricks harness for Silver-to-Gold Phase 13 sensitivity validation."""
...
PHASE13_TARGET_CONFIG = {
    "selection_sensitivity_results": {
        "build_order": 200,
        "reconciliation_name": "selection_sensitivity_results_row_balance",
    },
    "recommendation_explanations": {
        "build_order": 210,
        "reconciliation_name": "recommendation_explanations_row_balance",
    },
}
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Validate Silver-to-Gold Phase 13 sensitivity and explainability in Databricks."
    )
...
def main() -> None:
    ...
    try:
        ...
        for table_name in PHASE13_TARGET_TABLES:
            started_ts = utc_now()
            ...
            append_records(
                ...
                [
                    build_table_run_start_record(
                        ...
                        build_stage="sensitivity",
                        build_order=PHASE13_TARGET_CONFIG[table_name]["build_order"],
                        started_ts=started_ts,
                    )
                ],
            )

        publication_summary = publish_phase13_sensitivity_products(
            spark,
            environment,
        ...
        source_counts = {
            "selection_sensitivity_results": publication_summary.selection_sensitivity_results.input_row_count,
            "recommendation_explanations": publication_summary.recommendation_explanations.input_row_count,
        }
        output_counts = {
            "selection_sensitivity_results": publication_summary.selection_sensitivity_results.output_row_count,
            "recommendation_explanations": publication_summary.recommendation_explanations.output_row_count,
        }
        excluded_counts = {
        ...
        append_records(
            ...
            [
                build_reconciliation_record(
                    pipeline_context,
                    reconciliation_name=PHASE13_TARGET_CONFIG[table_name]["reconciliation_name"],
        ...
        print("Validated Phase 13 source columns:")
        for table_name, columns in validated_columns.items():
            print(f"  - {environment.catalog}.{environment.gold_schema}.{table_name}")
            for column_name in columns:
                print(f"      * {column_name}")
        print("Planned Phase 13 target tables:")
        for table_name in PHASE13_TARGET_TABLES:
            print(f"  - {environment.catalog}.{environment.gold_schema}.{table_name}")
        print("Published Phase 13 target tables:")
        print(
            f"  - selection_sensitivity_results: "
            f"{publication_summary.selection_sensitivity_results.target_table_fqn} "
            f"(input={source_counts['selection_sensitivity_results']}, "
            f"output={output_counts['selection_sensitivity_results']})"
        )
        print(
            f"  - recommendation_explanations: "
            f"{publication_summary.recommendation_explanations.target_table_fqn} "
            f"(input={source_counts['recommendation_explanations']}, "
            f"output={output_counts['recommendation_explanations']})"
        )
        ...
        set_task_value(
            dbutils,
            "phase13_selection_sensitivity_results_row_count",
            output_counts["selection_sensitivity_results"],
        )
        set_task_value(
            dbutils,
            "phase13_recommendation_explanations_row_count",
            output_counts["recommendation_explanations"],
        )
