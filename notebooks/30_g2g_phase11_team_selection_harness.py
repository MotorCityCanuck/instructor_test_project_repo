"""Databricks harness for Silver-to-Gold Phase 11 team selection validation."""
...
PHASE11_TARGET_CONFIG = {
    "team_selection_scorecards": {
        "build_order": 170,
        "reconciliation_name": "team_selection_scorecards_row_balance",
    },
    "olympic_team_candidates": {
        "build_order": 180,
        "reconciliation_name": "olympic_team_candidates_row_balance",
    },
}
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Validate Silver-to-Gold Phase 11 team selection in Databricks."
    )
...
def main() -> None:
    ...
    try:
        ...
        validated_columns = validate_phase11_source_contract(spark, environment)
        ...
        for table_name in PHASE11_TARGET_TABLES:
            started_ts = utc_now()
            ...
            append_records(
                ...
                [
                    build_table_run_start_record(
                        ...
                        build_order=PHASE11_TARGET_CONFIG[table_name]["build_order"],
                        started_ts=started_ts,
                    )
                ],
            )

        publication_summary = publish_phase11_selection_tables(
            spark,
            environment,
        ...
        source_counts = {
            "team_selection_scorecards": publication_summary.team_selection_scorecards.input_row_count,
            "olympic_team_candidates": publication_summary.olympic_team_candidates.input_row_count,
        }
        output_counts = {
            "team_selection_scorecards": publication_summary.team_selection_scorecards.output_row_count,
            "olympic_team_candidates": publication_summary.olympic_team_candidates.output_row_count,
        }
        excluded_counts = {
        ...
        append_records(
            ...
            [
                build_reconciliation_record(
                    pipeline_context,
                    reconciliation_name=PHASE11_TARGET_CONFIG[table_name]["reconciliation_name"],
        ...
        print("Validated Phase 11 source columns:")
        for logical_name, columns in validated_columns.items():
            layer, table_name, _required_columns = PHASE11_REQUIRED_SOURCE_COLUMNS[logical_name]
            ...
            for column_name in columns:
                print(f"      * {column_name}")
        print("Planned Phase 11 target tables:")
        for table_name in PHASE11_TARGET_TABLES:
            print(f"  - {environment.catalog}.{environment.gold_schema}.{table_name}")
        print("Published Phase 11 target tables:")
        print(
            f"  - team_selection_scorecards: "
            f"{publication_summary.team_selection_scorecards.target_table_fqn} "
            f"(input={source_counts['team_selection_scorecards']}, "
            f"output={output_counts['team_selection_scorecards']})"
        )
        print(
            f"  - olympic_team_candidates: "
            f"{publication_summary.olympic_team_candidates.target_table_fqn} "
            f"(input={source_counts['olympic_team_candidates']}, "
            f"output={output_counts['olympic_team_candidates']})"
        )
        ...
        set_task_value(
            dbutils,
            "phase11_team_selection_scorecards_row_count",
            output_counts["team_selection_scorecards"],
        )
        set_task_value(
            dbutils,
            "phase11_olympic_team_candidates_row_count",
            output_counts["olympic_team_candidates"],
        )
