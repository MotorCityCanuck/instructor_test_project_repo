...
def test_build_organization_sql_plan_for_clubs_contains_expected_rules() -> None:
    ...
    assert "region_country_code" in plan.accepted_sql
...
def test_build_organization_sql_plan_for_clubs_uses_actual_bronze_columns() -> None:
    ...
    assert "COALESCE(country_code, region_country_code) AS country_code" in combined_sql
...
def test_build_organization_sql_plan_for_teams_contains_expected_rules() -> None:
    ...
    assert "team_name" not in plan.accepted_sql
...
def test_build_organization_sql_plan_for_club_memberships_uses_end_date_when_source_native_columns_are_present() -> None:
    config, environment, context = _config_environment_context()
...
def test_build_organization_sql_plan_for_team_memberships_uses_joined_date_when_source_native_columns_are_present() -> None:
    ...
    assert "CAST(membership_start_date AS STRING)" not in combined_sql
    assert "CAST(membership_end_date AS STRING)" not in combined_sql
