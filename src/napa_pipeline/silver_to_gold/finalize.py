"""Pipeline finalization helpers for the Silver-to-Gold workflow."""
...
class PipelineRunSummary:
    """Aggregate summary for one Silver-to-Gold pipeline run."""
...
class PipelineFinalizationError(RuntimeError):
    """Raised when Gold pipeline finalization cannot be completed."""
...
def summarize_pipeline_run(
    ...
    """Summarize published Gold outcomes for one pipeline run."""
    pipeline_run_rows = [
        row
    ...
    if not table_run_rows and not reconciliation_rows and not pipeline_run_rows:
        raise PipelineFinalizationError(
            "No pipeline_runs, gold_table_runs, or gold_reconciliation_results were found for "
    ...
    if (
        ...
        or failed_reconciliation_count != 0
...
def finalize_pipeline_run(
    ...
    """Finalize the durable shared pipeline-run record."""
...
def _build_summary_text(
    *,
    ...
    if final_status == "SUCCEEDED":
        return f"Silver-to-Gold pipeline succeeded for {expected_table_count} configured tables."

    details = [
        "Silver-to-Gold pipeline failed: "
        ...
        f"failed_reconciliations={failed_reconciliation_count}, "
    ...
    if critical_quality_details:
        details.append(
            "Critical quality failures: " + "; ".join(critical_quality_details[:10]) + "."
        )
...
def _format_failed_table_detail(row: dict[str, Any]) -> str:
    table_name = row.get("target_gold_table") or "<unknown_table>"
